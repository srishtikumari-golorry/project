...
dataset = pd.merge(placement.loc[:,["Order ID","Weight"]],order.loc[:,["ID","Shipper ID","Requested Weight"]],how="left",left_on='Order ID',right_on='ID',suffixes=("",""))
print(dataset.info())
dataset.dropna(axis=0,inplace=True)
dataset=pd.DataFrame(dataset.groupby("Shipper ID").agg({"Weight":'sum',"Requested Weight":'sum'}))
